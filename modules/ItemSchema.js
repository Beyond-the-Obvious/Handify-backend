...
function deleteItem(req, res) {
    ...
    ItemModel.findByIdAndDelete(id).then(record => {
        res.send(record);
    }).catch(error => {
...
function updateItem(req, res) {
    const id = req.params.id;
    const { data } = req.body;
    ItemModel.findByIdAndUpdate(id, data).then(record => {
        res.send(record);
    }).catch(error => {
        res.status(500).send(error.message)
    })
...
const test1 = new ItemModel({
    ...
    price: '100000',
    imgURL: 'https://thumbs.dreamstime.com/b/dollar-sign-white-background-money-gold-symbol-d-illustration-savings-isolated-dollar-sign-white-background-buy-gold-currency-139878542.jpg'
})
